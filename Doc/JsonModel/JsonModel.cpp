...
QString JsonModel::xyvSurferData() const
{
    ...
    Q_FOREACH(JsonModelItem *item, topLevelItemsList)
    {
        ...
        JsonTableModel *model = 0;
        if (data.canConvert<JsonTableModel*>())
            model = data.value<JsonTableModel*>();
...
QVariant JsonModel::toVariant(const QString &parentKey, const QString &key, const QJsonValue &value)
{
    ...
    {
        JsonTableModel *tableModel = new JsonTableModel(const_cast<JsonModel*>(this));
...
struct JsonTableModelData
{
    ...
    QString recordName;
    double timeFieldMinValue;
    double timeFieldMaxValue;
    double resistivityFieldMinValue;
    double resistivityFieldMaxValue;
    JsonModel *jsonModel;
    QList< QVariantList > data;
};

JsonTableModel::JsonTableModel(JsonModel *jsonModel) : QAbstractTableModel(jsonModel)
{
    d = new JsonTableModelData;
...
QString JsonTableModel::description() const
{
    return QString("Table with %1 rows").arg(d->data.size());
}

void JsonTableModel::setRecordName(const QString &recordName)
{
...
QString JsonTableModel::recordName() const
{
    return d->recordName;
}

double JsonTableModel::minValue(int column) const
{
    if(column < 0 || column > d->columnNames.count())
        return  0;

    if(d->columnNames.at(column) == "TIME")
        return d->timeFieldMinValue;

    if(d->columnNames.at(column) == "RESISTIVITY")
        return d->resistivityFieldMinValue;

    return 0;
}

double JsonTableModel::minValue(const QString &columnName) const
{
    int columnIndex = d->columnNames.indexOf(columnName);

    if(columnIndex == -1)
        return 0;

    return minValue(columnIndex);
}

double JsonTableModel::maxValue(int column) const
{
    if(column < 0 || column > d->columnNames.count())
        return  0;

    if(d->columnNames.at(column) == "TIME")
        return d->timeFieldMaxValue;

    if(d->columnNames.at(column) == "RESISTIVITY")
        return d->resistivityFieldMaxValue;

    return 0;
}

double JsonTableModel::maxValue(const QString &columnName) const
{
    int columnIndex = d->columnNames.indexOf(columnName);
    ...
    return maxValue(columnIndex);
}

void JsonTableModel::addRows(const QJsonArray &array)
{
    ...
    for(int i=0; i<array.count(); i++)
    {
        ...
        Q_FOREACH(QString key, columnNames)
        {
            ...
            if(firstIteration )
            {
                if(key == "TIME")
                {
                    d->timeFieldMinValue = value;
                    d->timeFieldMaxValue = value;
                }
            }
            else
            {
                if(key == "TIME")
                {
                    if(value < d->timeFieldMinValue)
                        d->timeFieldMinValue = value;

                    if(value > d->timeFieldMaxValue)
                        d->timeFieldMaxValue = value;
                }
            }
...
void JsonTableModel::removeRowAt(int row)
{
...
QJsonArray JsonTableModel::jsonArray() const
{
    QJsonArray ret;
...
QStringList JsonTableModel::columnNames() const
{
...
QVariant JsonTableModel::value(int row, const QString &columnName) const
{
    return this->value(row, d->columnNames.indexOf(columnName));
}

bool JsonTableModel::setValue(int row, const QString &columnName, const QVariant &value)
{
    return this->setValue(row, d->columnNames.indexOf(columnName), value);
}

QVariant JsonTableModel::value(int row, int col) const
{
    if(row < 0 || row >= d->data.size())
        return QVariant();
...
bool JsonTableModel::setValue(int row, int col, const QVariant &value)
{
    if(row < 0 || row >= d->data.size())
        return false;
...
void JsonTableModel::onJsonDoucumentModified()
{
    QList<JsonModelItem*> topLevelItems = d->jsonModel->topLevelItems();

    double coilSize = 0;
    double loopSize = 0;
    double current = 0;

    double powerFactor1 = (double)2/3;
    double powerFactor2 = -((double)5/3);

    Q_FOREACH(JsonModelItem *item, topLevelItems)
    {
        if(item && item->key() == d->recordName)
        {
            QString searchField = d->recordName + ".Header.";
            if(item->itemForFieldName(searchField + "COIL_SIZE"))
                coilSize = item->itemForFieldName(searchField + "COIL_SIZE")->value().toDouble();

            if(item->itemForFieldName(searchField +  + "LOOP_SIZE.X") &&
                    item->itemForFieldName(searchField + "LOOP_SIZE.Y"))
                loopSize = item->itemForFieldName(searchField +  "LOOP_SIZE.X")->value().toDouble() *
                        item->itemForFieldName(searchField + "LOOP_SIZE.Y")->value().toDouble();

            if(item->itemForFieldName(searchField + "CURRENT"))
                current = item->itemForFieldName(searchField + "CURRENT")->value().toDouble();
        }
    }

    d->columnNames.append("RESISTIVITY");
    d->columnNames.append("TIME/DEPTH");

    int timeValIndex = d->columnNames.indexOf("TIME") + 1;
    int voltageValIndex = d->columnNames.indexOf("VOLTAGE") + 1;

    bool firstIteration = true;
    this->beginResetModel();
    for(int i=0; i<d->data.count(); i++)
    {
        QVariant row = d->data[i];
        QList<QVariant> values = row.toList();
        double time = values.at(timeValIndex).toDouble();
        double voltage = values.at(voltageValIndex).toDouble();

        double resisitiviy = 6.32 * qPow(voltage/(coilSize*loopSize*current), powerFactor1) *
                qPow(time, powerFactor2);

        if(firstIteration)
        {
            d->resistivityFieldMinValue = resisitiviy;
            d->resistivityFieldMaxValue = resisitiviy;
        }
        else
        {
            if(resisitiviy < d->resistivityFieldMinValue)
                d->resistivityFieldMinValue = resisitiviy;

            if(resisitiviy > d->resistivityFieldMaxValue)
                d->resistivityFieldMaxValue = resisitiviy;
        }

        values.append(resisitiviy);
        values.append(0);
